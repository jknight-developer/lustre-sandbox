...
pub fn index(model: Model) -> Element(Msg) {
  html.div([], [
    html.div([], [
      ...
      ui.centre(
        [classes.mt_md()],
        html.p([classes.font_mono(), classes.text_lg()], [
          element.text("lorem ipsum whatever man who cares"),
        ]),
      ),
      html.div([attribute.style([#("display", "flex")])], [
        ui.button([event.on_click(IntMessage(msg.IntDecrement("icons")))], [
          element.text("-"),
        ]),
        ui.button([event.on_click(IntMessage(msg.IntIncrement("icons")))], [
          element.text("+"),
        ]),
      ]),
    ]),
    ...
    html.div([], [
      ...
      ui.centre(
        ...
        ui.aside(
          [],
          lib.input_box(model, "name", "[your text here]", [
            classes.text_2xl(),
            attribute.style([#("max-width", "50%")]),
            input.primary(),
          ]),
          ui.centre(
            [],
