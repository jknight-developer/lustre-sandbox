...
pub fn about(model: Model) -> Element(Msg) {
  html.div([attribute.style([#("min-height", "80rem")])], [
    ...
    ui.centre(
      [button.warning(), classes.pb_lg()],
      ui.button([event.on_click(msg.IntMessage(msg.IntIncrement("fizzbuzz")))], [
        html.p([classes.font_alt(), classes.text_5xl()], [
          element.text("MORE POWER"),
        ]),
      ]),
    ),
    html.div(
      ...
      fizzbuzz.fizzbuzz(result.unwrap(
        dict.get(model.state.ints, "fizzbuzz"),
        10,
      )),
    ),
  ])
